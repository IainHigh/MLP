...
class LSTM_regr(torch.nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim):
        ...
        self.linear = nn.Linear(
            hidden_dim, 1
        )  # TODO: Try wth different number of out features
...
def train_model_regr(model, train_dl, val_dl, epochs=10, lr=0.001):
    ...
    for i in range(epochs):
        ...
        if i % 10 == 0:
            ...
            model.train()
            print(
                "Epoch %d | Train RMSE %.3f | Train MAE %.3f | Val RMSE %.3f | Val MAE %.3f"
                % (i, train_rmse, train_mae, val_rmse, val_mae)
            )
    return model
...
def calculate_mae(model, valid_dl):
    ...
    with torch.no_grad():
        for x, y, l in valid_dl:
            ...
            absolute_error_sum += (
                torch.abs(y_hat.squeeze() - y).sum().item()
            )  # Ensure y_hat is squeezed to match y's shape
...
def main():
    if not preprocessed:
        ...
        # preprocess data
        processed_train_df, processed_val_df, processed_test_df, words = (
            preprocess_data(train_df, val_df, test_df, mod_books_df)
        )

        # save processed dataframes
        processed_train_df.to_csv(
            "/exports/eddie/scratch/s2062378/MLP_Dataset/processed_train_df.csv",
            index=False,
        )
        processed_val_df.to_csv(
            "/exports/eddie/scratch/s2062378/MLP_Dataset/processed_val_df.csv",
            index=False,
        )
        processed_test_df.to_csv(
            "/exports/eddie/scratch/s2062378/MLP_Dataset/processed_test_df.csv",
            index=False,
        )
    ...
    # get training set
    X_train = list(processed_train_df)
    y_train = list(processed_train_df["time_to_start_seconds"])

    # get validation set
    X_valid = list(processed_val_df)
